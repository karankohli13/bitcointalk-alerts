...
async def parse_topic(url):
    ...
    text = text.replace('Today at', pendulum.now().to_date_string())
    try:
        created = pendulum.parse(text)
    except Exception as e:
        print('sad')
    try:
        text =  h.xpath('//div[@class="subject"]//following-sibling::div/span[1]')[0].text_content()
        print(text)
        created = pendulum.parse(text,strict=False)
    except Exception as e:
        text = text.replace('Today at', '')
        created = pendulum.parse(text,strict=False)
    await db.execute(
